...
<ul class="results" id="classmatesList">
  {foreach $results['people'] as $item}
    {if !isset($item['separator'])}
      <li{if $item['img']} class="icon"{/if}>
		...
		{/capture}
		{block name="itemLink"}
		  <a href="detail?keyword={$results['keyword']}&amp;id={$item['id']}" class="{$item['class']|default:''}"{if $linkTarget || $item['linkTarget']} target="{if $item['linkTarget']}{$item['linkTarget']}{else}{$linkTarget}{/if}"{/if}>
		    {if $item['img']}
